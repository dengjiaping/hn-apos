...
public class TxnFlowPurchaseFaildTransfer implements TiFlowNodeDataTransfer {
	...
	public Map<String, String> transfterData(Activity activity,
		...
		PostVoucherContext postVoucherContext = TiFlowControlImpl
				.instanceControl().getFlowContextData(PostVoucherContext.class);
		...
		TiFlowControlImpl.instanceControl().setFlowContextData(
				purchaseOrderFaildContext);
		purchaseOrderFaildContext.setPaymentMethod(PaymentMethods.SWIPING);
		purchaseOrderFaildContext.setReceiptNo(postVoucherContext
				.getReceiptNo());
		purchaseOrderFaildContext.setShoppingCart(postVoucherContext
				.getShoppingCart());
		purchaseOrderFaildContext.setShowOutButton(false);
		purchaseOrderFaildContext
				.setMsgContent("网络不给力，请重新提交订单,在提交成功前，不要强制退出此页面。");
		purchaseOrderFaildContext.setTxnId(postVoucherContext.getTxnId());
		purchaseOrderFaildContext.setOrderTime(postVoucherContext
				.getOrderTime());
		return null;
