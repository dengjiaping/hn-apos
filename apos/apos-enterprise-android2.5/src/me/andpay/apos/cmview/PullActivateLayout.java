...
public class PullActivateLayout extends FrameLayout {
	...
	public PullActivateLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.PullActivateLayout, defStyle, 0);
	...
	/**
	 * Replace the ActionView by code. The ActionView will also be add as a
	 * child.
	 * 
	 * @param view
	 *            - The ActionView
	...
	public void scrollTo(int x, int y) {
		...
		if (new BigDecimal(destY).abs().intValue() > this.mActionViewHeight) {
			super.scrollTo(x, 0 - this.mActionViewHeight - 10);
		} else {
	 ...
	 * 
	 * @param enable
	 *            - True if the actionView can partial show, false otherwise.
	 */
	...
	/**
	 * Set a listener to monitor the Pull out/in state. The listener will be
	 * call on pull state changed.
	 * 
	 * @param listener
	 *            - The OnPullStateListener
	 */
	...
	/**
	 * Indicates whether all children view has been pull down under the top of
	 * layout. <a
	 * href="\"http://www.eoeandroid.com/home.php?mod=space&uid=7300\""
	 * target="\"_blank\"">@return</a> true if all children view is pull down,
	 * false otherwise
	 */
