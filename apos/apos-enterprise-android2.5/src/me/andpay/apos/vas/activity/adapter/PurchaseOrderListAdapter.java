 ...
 * 
 */
public class PurchaseOrderListAdapter extends
		TiSectionListAdapter<PurchaseOrderInfo> {
	...
	public View getSectionItemView(int sectionIndex, int itemIndex,
		...
		holder.payTypeImg
				.setImageResource(PaymentMethods.CASH.equalsIgnoreCase(info
		...
		for (int i = 0; i < info.getItems().size(); i++) {
			...
			descStr.append(item.getProductName() == null ? "" : item
					.getProductName());
		...
		holder.descView.setText(descStr.length() > MAXDESCSIZE ? descStr
				.toString().substring(0, MAXDESCSIZE) + ELLIPSIS : descStr
				.toString());
		holder.amountView.setText(StringConvertor.convert2Currency(info
				.getSalesAmt()));
		holder.timeView.setText(DateUtil.format(TIME_PARTTERN,
				info.getOrderTime()));
		if (PurchaseOrder.STATUS_REFUND.equals(info.getStatus())) {
			holder.refundImg.setVisibility(View.VISIBLE);
			holder.amountView.setTextColor(applicationContext.getResources()
					.getColor(R.color.com_cccccc_col));
			holder.timeView.setTextColor(applicationContext.getResources()
					.getColor(R.color.com_cccccc_col));
		} else {
			holder.refundImg.setVisibility(View.GONE);
			holder.amountView.setTextColor(applicationContext.getResources()
					.getColor(R.color.com_666666_col));
			holder.timeView.setTextColor(applicationContext.getResources()
					.getColor(R.color.com_666666_col));
		}
		return convertView;
	...
	public String getSectionDesc(PurchaseOrderInfo info) {
		return DateUtil.format(DATE_PARTTERN, info.getOrderTime())
				+ " "
				+ me.andpay.timobileframework.util.DateUtil
						.getWeekChineseDesc(info.getOrderTime());
	}
