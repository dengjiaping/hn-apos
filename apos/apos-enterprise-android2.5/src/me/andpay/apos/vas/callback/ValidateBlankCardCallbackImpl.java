...
public class ValidateBlankCardCallbackImpl implements ValidateBlankCardCallback {
	...
	public void netWorkError() {
		...
		final PromptDialog dialog = new PromptDialog(
				openCardRegisterMainActivity, "提示",
				"网络异常，请稍后重试,在网络恢复前请不要退出此页面。");
		dialog.setCancelable(false);

		dialog.setSureButtonOnclickListener(new OnClickListener() {
			public void onClick(View v) {
				dialog.dismiss();
				openCardRegisterMainActivity.oprCardRegisterMainControl
						.submit(openCardRegisterMainActivity);
			}
		});
		dialog.show();
	...
	public void validateSuccess() {
		...
		TiFlowControlImpl.instanceControl().nextSetup(
				openCardRegisterMainActivity, FlowConstants.SUCCESS);
	}
	...
	public void validateFaild(String errorMsg) {
		...
		TiFlowControlImpl.instanceControl().nextSetup(
				openCardRegisterMainActivity, FlowConstants.FAILED, intentData);
	}
