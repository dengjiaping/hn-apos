...
public class SwipeTxnFlowOpenCardTransfer implements TiFlowNodeDataTransfer {
	...
	public Map<String, String> transfterData(Activity activity,
		...
		TxnContext txnContext = (TxnContext) subFlowContext
				.get(TxnContext.class.getName());
		...
		if (txnContext.getOrderId() != null) {
			...
			// PurchaseOrderFaildContext purchaseOrderFaildContext =
			// (PurchaseOrderFaildContext)subFlowContext.get(PurchaseOrderFaildContext.class.getName());
			// openCardContext.setOrderId(purchaseOrderFaildContext.getOrderId());
		}
		openCardContext.setCardQuantity(productSalesContext.getShoppingCart()
				.getTotalProduct());
		openCardContext.setProductType(productSalesContext.getShoppingCart()
				.getProductType());
