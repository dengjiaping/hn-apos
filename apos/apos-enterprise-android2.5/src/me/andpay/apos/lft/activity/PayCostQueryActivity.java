...
public class PayCostQueryActivity extends AposBaseActivity {
	...
	public void deatail(View v){
		PayCostType type = (PayCostType)TiFlowControlImpl.instanceControl().getFlowContext().get("type");
		if(type==PayCostType.ELECTRICITY){//电费详情
			TiFlowControlImpl.instanceControl().nextSetup(this,FlowConstants.ELE_DEATAIL);
		}else if(type==PayCostType.WATER){//水费详情
			TiFlowControlImpl.instanceControl().nextSetup(this,FlowConstants.WATER_DEATAIL);
		}
	}
