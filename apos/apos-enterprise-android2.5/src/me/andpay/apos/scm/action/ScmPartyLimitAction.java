...
public class ScmPartyLimitAction  extends SessionKeepAction {
	...
	public ModelAndView queryPartyLimit(ActionRequest request) {
		...
		ScmPartyLimitCallBack scmPartyLimitCallBack = (ScmPartyLimitCallBack) request.getHandler();
		
		try {
			PartyTxnAmtQuotaInfo partyLimit = rcsService.queryPartyTxnAmtQuota();
			scmPartyLimitCallBack.querySuccess(partyLimit);
		}catch(Exception ex) {
			scmPartyLimitCallBack.queryError(ErrorMsgUtil.parseError(application, ex));
		}
