...
public class IDCardUtil {
	...
	// private static Boolean cacheValidateResult = null;
	// // 缓存出生日期，因为出生日期使用频繁且计算复杂
	// private static Date cacheBirthDate = null;
	...
	public String getAddressCode(String cardNumber) {
	...
	public static Date getBirthDate(String cardNumber) {
		...
		try {
			Date cacheBirthDate = createBirthDateParser().parse(
	...
	public boolean isFemal(String cardNumber) {
		return false == this.isMale(cardNumber);
	}
	...
	private static void checkIfValid(String cardNumber) {
		if (false == validate(cardNumber)) {
